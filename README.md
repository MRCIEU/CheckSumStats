...
The first step is to map the reported trait with the Experimental Factor Ontology (EFO), using the get\_efo function. The function identifies EFOs using the [ZOOMA REST API](https://www.ebi.ac.uk/spot/zooma/).
...
Each red data point corresponds to an allele frequency conflict and is identified for approximately half of the SNPs. This pattern occurs when minor allele frequency is interpreted as effect allele frequency but the effect allele is not always the minor allele.
...
[ZOOMA](https://www.ebi.ac.uk/spot/zooma/) [NHGRI-EBI GWAS catalog](https://www.ebi.ac.uk/gwas/)
